...
## Previous part recap
Before proceeding, first let's have a quick recap of system under monitoring from previous post.
This is a simple task ticketing (like Jira) service written on top of Akka HTTP and Slick and provides very basic CRUD REST API for tickets management.

![](images/diagram_system_architecture.drawio.png)

It uses following external services:
...
- `project-service` to get information about project to which ticket belongs to (mocked service);

For this service we would like to monitor couple simple things:
- Number of tickets in the system;
- Traces for each request;

In order to simulate user traffic (create, read, update and delete tickets) Gatling was used.
...
Kamon provides ,out of the box, number of instrumentation's like: [Akka HTTP](https://kamon.io/docs/latest/guides/installation/akka-http/), [Spring Boot](https://kamon.io/docs/latest/guides/installation/spring-boot/), [Play Framework](https://kamon.io/docs/latest/guides/installation/play-framework/) and [Lagom](https://kamon.io/docs/latest/guides/installation/lagom-framework/).
...
### How to plug instrumentation to application
For particularly example case, we interested in Akka instrumentation, which is described [here](https://kamon.io/docs/latest/guides/installation/akka-http/)
NOTE: At the moment of writing this blog post, used version of Kamon is `2.6.0`. Latest version can be found [here](https://mvnrepository.com/artifact/io.kamon)
...
### Metrics
Kamon provides number of [metrics](https://kamon.io/docs/latest/core/metrics/) in its API.
...
So to track number of created tickets per project we can use following code:
```scala
...
Here we will use Prometheus to monitor metrics of our application.
...
Among response headers you can find value `trace-id` - `1b05566f51be20ad` which is a trace id of our request.
On Zipkin UI at `localhost:9411` we can find our trace:
![screenshot_kamon_zipkin.png](images/screenshot_kamon_zipkin.png)

This trace includes all requests made by our application, including requests to `projects-service`, query to Postgres etc.
...
But, among instrumentation's and libraries, Kamon also provides commercial product called [Kamon APM](https://kamon.io/docs/latest/reporters/apm/). 
...
It is a bit hard to choose between OpenTelemetry and Kamon as telemetry solution for Scala & Lightbend stack, because they provide similar functionality in very handy way, such as automatic instrumentation.
...
Kamon does not provide mechanism for logs sending to external service, like OpenTelemetry does with [OTLP Protocol](https://opentelemetry.io/docs/reference/specification/logs/).
But, there is plenty of ways to do this with other tools.
<br>List of [OpenTelemetry's JVM instrumentation's](https://github.com/open-telemetry/opentelemetry-java-instrumentation/blob/main/docs/supported-libraries.md#libraries--frameworks) is certainly bigger than of [Kamon's](https://kamon.io/docs/latest/instrumentation/).
...
If you are using multiple languages across your organization, or you have existing complex monitoring solution, then it's probably better to stick with OpenTelemetry, as it's language and vendor agnostic.
But, if Scala and Akka ecosystem is your primary choice, then Kamon might be a better fit for you.

That's it for this time. At next post we will talk about [Trace4Cats](https://github.com/trace4cats/trace4cats/) TypeLevel stack based projects. 
